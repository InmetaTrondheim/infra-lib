    ...
    let
      ...
      mynixvim =
        nixvim.legacyPackages.x86_64-linux.makeNixvimWithModule
          {
            inherit pkgs; module = { imports = [ mynvim.nixvimModules.nixVim { langs.terraform.enable = true; } ]; };
          };

      myShell = pkgs.mkShell {
        buildInputs = with pkgs; [ nixd opentofu ];
        packages = [ mynixvim (pkgs.azure-cli.withExtensions [ pkgs.azure-cli.extensions.azure-devops ]) ];
      };
    in
    {
